			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				if(typeof this.items[i] != undefined){
					...
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="'+ this.config.baseUrl +'img/ajax-loader.gif" /></div></div>';
				}
			}
				...
				success:function(rponse){
					...
					$.each(obj,function(k,fle){
						if($(fle).attr("rel") == rponse){
							...
							$('.upload-status').append('File uploaded<br/>');
						}
					});
					if (f+1 < file.length) {
