...
class UploadServiceComponent extends Component {
    ...
    */
    ...
    protected function get_file_object($file_name) {
        ...
        if (is_file($file_path) && $file_name[0] !== '.') {
            ...
            $upload = $this->UploadModel->find('first', array(
            		'conditions' => array('name' => $file_name)
            ));
            $file->uploader = $upload['User']['email'];
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
        ...
        if (!$error && $file->name) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                //TODO save project and experiment id when upload files   
                $data = array(
                    'Upload' => array(
                        'name' => $name,
                        'size' => $size,
                    	'project_id' => $_SESSION['Project']['id'],
                    	'experiment_id' => $_SESSION['Experiment']['id'],	
    ...
    public function get($files = null) {
        ...
        if(!empty($files)){
        	foreach($files as $file){
        		$info[] = $this->get_file_object($file['name']);
        	}
        }
        /*
        if ($file_name) {
